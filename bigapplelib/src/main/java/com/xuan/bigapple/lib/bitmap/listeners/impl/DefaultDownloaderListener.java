...
public class DefaultDownloaderListener implements DownloaderListener {
    ...
    private static final int CONNECT_TIMEOUT = 1000 * 15;// 15S
    private static final int READ_TIMEOUT = 1000 * 30;// 30S

    /**过期时长,单位毫秒.默认无限大没有过期时间*/
    private long defaultExpiry = Long.MAX_VALUE;

    /**
     * 把网络或本地图片下载到文件的 outputStream
     *
     * @param uri          加载图片地址，('/'打头的加载本地，其余从网络中加载)
     * @param outputStream 加载输出流
     * @param callback     加载中回调
